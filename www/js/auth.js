...
function getCurrentUser() {
  return sessionStorage.getItem('pixel_username') || '';
}
function isLoggedIn() {
...
function getUsers() {
  try { // [v3.2 SQLite] users stored in database
      console.warn("getUsers() called in SQLite mode");
      return {}; }
  catch(e) { return {}; }
}
function saveUsers(users) {
  // [v3.2 SQLite] users stored in database
}
function getCurrentUser() {
  return sessionStorage.getItem('pixel_username') || '';
}
...
async function handleAuth() {
  ...
  if (authIsRegister) {
    ...
    if (!regR.ok) {
      errEl.textContent = (regR.error) || '注册失败'; errEl.classList.add('show'); return;
    }
    apiSetUser(regR.user_id);
    sessionStorage.setItem('pixel_username', user);
    ...
    if (!loginR.ok) {
      errEl.textContent = (loginR.error) || '登录失败'; errEl.classList.add('show'); return;
    }
...
// ==================== 问卷系统 ====================
var quizSteps = [
  { q: '你的身份是？', opts: ['学生', '上班族', '自由职业', '老师', '其他'], key: 'identity' },
  { q: '你的主要目标？', opts: ['提高效率', '健康管理', '学习规划', '工作安排', '综合'], key: 'goal' },
  { q: '你常用的标签偏好？（可多选）', opts: ['工作', '学习', '运动', '社交', '娱乐', '健康', '出行'], key: 'prefTags', multi: true }
];
var quizIdx = 0;
var quizAnswers = {};
var quizMultiSel = [];
